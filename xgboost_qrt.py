...
# ==========================
for allocation in tqdm(X_train['ALLOCATION'].unique()[:3]):
    ...
    def objective(trial):
        ...
        if mean > best_mean:
            ...
            plot_scores_xgb[allocation] = (mean, std)
...
allocations = list(plot_scores_xgb.keys())
means = [plot_scores_xgb[alloc][0] for alloc in allocations]
stds = [plot_scores_xgb[alloc][1] for alloc in allocations]

plt.figure(figsize=(10, 6))
plt.bar(allocations, means, yerr=stds, capsize=5)
plt.hlines(y=50, xmin=-1, xmax=len(allocations), colors='r', linestyles='dashed', label='Baseline 50%')
plt.hlines(y=53.722, xmin=-1, xmax=len(allocations), colors='k', linestyles='dashed', label='Baseline 53.722%')
plt.xlabel('Allocation')
plt.ylabel('Accuracy (%)')
plt.title('XGBoost Accuracy by Allocation with Standard Deviation')
